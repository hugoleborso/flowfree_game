...
public class Plateau {
		...
		for (int i = 0; i<nbLignes ; i++) {
			...
			for (int j = 0; j <nbColonnes; j++) {
				...
				for( Couleur couleur : Couleur.class.getEnumConstants() ) {
					if (couleur.getDataCouleur().getPostionsPlots().contains(new Position(i,j))) {					
	...
	public Case getCaseAutrePlotByCouleur(Plot plotCourant) {
		...
		Position positionPossible1 = couleurPlot.getDataCouleur().getPostionsPlots().get(0);
		...
		if (!(casePotentielle1==maCase)){
			...
			Position positionPossible2 = couleurPlot.getDataCouleur().getPostionsPlots().get(1);
