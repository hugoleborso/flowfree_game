...
public class Plateau {
		...
		for (int i = 0; i<nbLignes ; i++) {
			...
			for (int j = 0; j <nbColonnes; j++) {
				...
				for( Couleur couleur : Couleur.class.getEnumConstants() ) {
					if (couleur.getDataCouleur().getPositionsPlots().contains(new Position(i,j))) {					
	...
	// Methode uniquement utilisee pour l'affichage lors des test, elle serait supprimee en prod
	...
	public Case getCaseAutrePlotByCouleur(Plot plotCourant) {
		...
		Position positionPossible1 = couleurPlot.getDataCouleur().getPositionsPlots().get(0);
		...
		if (!(casePotentielle1==maCase)){
			...
			Position positionPossible2 = couleurPlot.getDataCouleur().getPositionsPlots().get(1);
